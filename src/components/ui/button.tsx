...
import {Spinner as SpinnerIcon} from '@phosphor-icons/react/dist/ssr'
    ...
    return (
        <div>
            <Card theme={theme} ring={ring} rounded={rounded} active={active} aspect="square">
                <div className={`flex items-center justify-center ${sizeClass}`}>{pending ? <SpinnerIcon weight={weight} className="animate-spin" /> : <Icon weight={weight} />}</div>
            </Card>
        </div>
    )
}
